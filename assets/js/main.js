...
function select(el, all = false) {
  ...
  if (all) {
    return [...document.querySelectorAll(trimmedEl)];
  }

  return document.querySelector(trimmedEl);
}
...
for (const form of needsValidation) {
...
if (mainContainer) {
  setTimeout(() => {
    new ResizeObserver(() => {
      for (const getEchart of select(".echart", true)) {
